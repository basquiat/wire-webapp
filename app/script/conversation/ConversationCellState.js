...
z.conversation.ConversationCellState = (() => {
  ...
  const _accumulateSummary = (conversationEntity, prioritizeSelfMention) => {
    ...
    const {unreadSelfMentions, unreadMessages} = conversationEntity.unreadState();
  ...
  const _getStateGroupActivity = {
    ...
    match: conversationEntity => {
      ...
      return (
        conversationEntity.is_group() && conversationEntity.unreadState().unreadEvents.length > 0 && isExpectedType
      );
    },
  };
